...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RSAssetData)

const URSAssetData& URSAssetData::Get()
{
	return ULyraAssetManager::Get().GetAssetData();
}

#if WITH_EDITOR
void URSAssetData::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
...
EDataValidationResult URSAssetData::IsDataValid(FDataValidationContext& Context) const
{
...
FSoftObjectPath URSAssetData::GetAssetPathByName(const FName& AssetName) const
{
...
const FAssetSet& URSAssetData::GetAssetSetByLabel(const FName& Label) const
{
