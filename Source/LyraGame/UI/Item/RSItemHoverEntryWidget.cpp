...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RSItemHoverEntryWidget)

#define LOCTEXT_NAMESPACE "ItemHoverWidget"

URSItemHoverEntryWidget::URSItemHoverEntryWidget(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
    
}

void URSItemHoverEntryWidget::NativePreConstruct()
{
...
void URSItemHoverEntryWidget::RefreshUI(const URSItemInstance* ItemInstance)
{
	...
	FColor RarityColor = URSUIData::Get().GetRarityColor(ItemRarity);
	...
	UTexture2D* RarityTexture = URSUIData::Get().GetHoverRarityTexture(ItemRarity);
	...
	// Item Specifics
	if (const URSItemFragment_Equipable* EquipableFragment = ItemTemplate.FindFragmentByClass<URSItemFragment_Equipable>())
	{
		if (EquipableFragment->EquipmentType == EEquipmentType::Armor)
		{
			const URSItemFragment_Equipable_Armor* ArmorFragment = Cast<URSItemFragment_Equipable_Armor>(EquipableFragment);
		...
		{
			const URSItemFragment_Equipable_Weapon* WeaponFragment = Cast<URSItemFragment_Equipable_Weapon>(EquipableFragment);
		...
		{
			const URSItemFragment_Equipable_Utility* UtilityFragment = Cast<URSItemFragment_Equipable_Utility>(EquipableFragment);
		...
		const FRSGameplayTagStackContainer& StackContainer = ItemInstance->GetStatContainer();
		for (const FRSGameplayTagStack& Stack : StackContainer.GetStacks())
		{
			FString Left, Right;
		...
		else
		{
			for (int32 i = 0; i < (int32)ECharacterClassType::Count; i++)
			{
				...
				const FRSClassInfoEntry& ClassInfoEntry = URSClassData::Get().GetClassInfoEntry(CharacterClassType);
