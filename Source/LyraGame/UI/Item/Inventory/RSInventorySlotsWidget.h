...
#include "RSInventorySlotsWidget.generated.h"
...
UCLASS()
class URSInventorySlotsWidget : public UUserWidget
{
	GENERATED_BODY()
	
public:
	URSInventorySlotsWidget(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
private:
	...
	void FinishDrag();
	void OnInventoryEntryChanged(const FIntPoint& ItemSlotPos, URSItemInstance* ItemInstance, int32 InItemCount);

public:
	URSInventoryManagerComponent* GetInventoryManager() const { return InventoryManager; }
...
private:
	UPROPERTY()
	TArray<TObjectPtr<URSInventorySlotWidget>> SlotWidgets;

	UPROPERTY()
	TArray<TObjectPtr<URSInventoryEntryWidget>> EntryWidgets;

	UPROPERTY()
	TArray<TObjectPtr<URSInventoryValidWidget>> ValidWidgets;
